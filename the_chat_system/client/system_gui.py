...
# READ THIS: there are three amazing neural networks models, and we can switch between them in line 88 and line 315, 317; read the comments there
...
# and here we initialize the state, to make our conditions for client_state_machine fit
    ...
    # here we load our hand-writen digit recognition neural network model
    # Change the file name here to load v1, v2 or v3: mnist_cnn, mnist_cnn_v2, mnist_cnn_v3
    # change line 315 or 317 accordingly: read the comments there
        sml = keras.layers.TFSMLayer('mnist_cnn_v3', call_endpoint='serve')
        self.cnn_model = keras.Sequential([keras.Input((28, 28, 1)), sml])
    ...
    def show_tools(self):
        ...
        tk.Button(self.content_frame, text='try "recognizing" digit', command=self.predict_drawing, width=20).pack(pady=5)
    ...
    def predict_drawing(self):
        ...
        # when loading v3, run this line
        arr = 255.0 - arr
        # when loading v1 or v2, run this line
        # arr = arr / 255.0
        arr = arr.reshape((1, 28, 28, 1))

        # 3) Predict with your CNN
        preds = self.cnn_model.predict(arr)[0]  # shape (10,)
